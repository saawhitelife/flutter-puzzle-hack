...
/// {@template walkie_countdown}
/// Displays the countdown before the puzzle is started.
/// {@endtemplate}
class WalkieCountdown extends StatefulWidget {
  /// {@macro walkie_countdown}
  const WalkieCountdown({
    Key? key,
  ...
  @override
  State<WalkieCountdown> createState() => _WalkieCountdownState();
}

class _WalkieCountdownState extends State<WalkieCountdown> {
  ...
  Widget build(BuildContext context) {
    return AudioControlListener(
      ...
      child: BlocListener<WalkiePuzzleBloc, WalkiePuzzleState>(
        listener: (context, state) {
          ...
          if (state.status == WalkiePuzzleStatus.started) {
        ...
        child: ResponsiveLayoutBuilder(
          ...
          large: (_, __) => BlocBuilder<WalkiePuzzleBloc, WalkiePuzzleState>(
...
/// {@template walkie_countdown_seconds_to_begin}
/// Display how many seconds are left to begin the puzzle.
/// {@endtemplate}
@visibleForTesting
class WalkieCountdownSecondsToBegin extends StatefulWidget {
  /// {@macro walkie_countdown_seconds_to_begin}
  const WalkieCountdownSecondsToBegin({
    Key? key,
  ...
  @override
  State<WalkieCountdownSecondsToBegin> createState() =>
      _WalkieCountdownSecondsToBeginState();
}

class _WalkieCountdownSecondsToBeginState
    extends State<WalkieCountdownSecondsToBegin>
...
/// {@template walkie_countdown_go}
/// Displays a "Go!" text when the countdown reaches 0 seconds.
/// {@endtemplate}
@visibleForTesting
class WalkieCountdownGo extends StatefulWidget {
  /// {@macro walkie_countdown_go}
  const WalkieCountdownGo({Key? key}) : super(key: key);

  @override
  State<WalkieCountdownGo> createState() => _WalkieCountdownGoState();
}

class _WalkieCountdownGoState extends State<WalkieCountdownGo>
        ...
        child: FadeTransition(
          ...
          child: ScaleTransition(
            ...
            child: ScaleTransition(
              scale: inScale,
              child: Text(
                context.l10n.walkieCountdownGo,
                style: PuzzleTextStyle.countdownTime.copyWith(
                  fontSize: 100,
